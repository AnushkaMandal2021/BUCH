...
<div style="margin-left:25%">
 ...
 <?php
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM books WHERE B_NAME = '$search' || B_AUTHOR = '$search' || B_GENRE = '$search' || B_PUBLISHER = '$search'";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM books WHERE B_NAME = '$search' || B_AUTHOR = '$search' || B_GENRE = '$search' || B_PUBLISHER = '$search' LIMIT $offset, $no_of_records_per_page ";
        $res_data = mysqli_query($conn,$sql);
        echo '<div class="w3-container w3-teal w3-xlarge">';
        echo "<h2>You Have Searched For ".$search."!!";"</h2>";
        echo"</div>";  
