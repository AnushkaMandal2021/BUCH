...
<table>
  ...
  <?php
        ...
        $stat = "DISPATCHED";
        $total_pages_sql = "SELECT COUNT(*) FROM orders O INNER JOIN purchase P WHERE P.P_ID = O.P_ID AND O.STAT = '$stat'";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM orders O INNER JOIN purchase P WHERE P.P_ID = O.P_ID AND O.STAT = '$stat' LIMIT $offset, $no_of_records_per_page ";
        ...
        echo '</div>';
        echo '<font color="black"><h2 style="text-align:center">Track Orders</h2></font>';
...
<?php 
 ...
 echo "<td class = 'noborders'><a href='http://localhost/buch/Admin/Orders/trackordersdb.php?id=$id&bid=$bid'><p><button class='button'><span>Track</span></button></p></a></td>";
 echo "<td class = 'noborders'><a href='http://localhost/buch/Admin/Orders/deliveredordersdb.php?id=$id&bid=$bid'><p><button class='button'><span>Delivered</span></button></p></a></td>";
?>
  </tr>
<?php
   }
