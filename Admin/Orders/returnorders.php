...
<table>
  ...
  <?php
        ...
        $restat = "RETURN INITIATED";
        $refstat = "REFUND INITIATED";
        $refund = "RETURNED & REFUNDED";

        $total_pages_sql = "SELECT COUNT(*) FROM orders O INNER JOIN purchase P WHERE P.P_ID = O.P_ID AND (O.STAT = '$restat' || O.STAT = '$refstat' || O.STAT = '$refund')";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM orders O INNER JOIN purchase P WHERE P.P_ID = O.P_ID AND (O.STAT = '$restat' || O.STAT = '$refstat' || O.STAT = '$refund') LIMIT $offset, $no_of_records_per_page ";
        ...
        echo '</div>';
        echo '<font color="black"><h2 style="text-align:center">Return Orders</h2></font>';
...
<?php 
if( $row["STAT"] == "RETURN INITIATED")
{
 echo "<td class = 'noborders'><a href='http://localhost/buch/Admin/Orders/returnordersdb.php?id=$id&bid=$bid'><p><button class='button'><span>INITIATE REFUND</span></button></p></a></td>";
}
if( $row["STAT"] == "REFUND INITIATED")
{
 echo "<td class = 'noborders'><a href='http://localhost/buch/Admin/Orders/returneddb.php?id=$id&bid=$bid'><p><button class='button'><span>RETURNED</span></button></p></a></td>";
}
