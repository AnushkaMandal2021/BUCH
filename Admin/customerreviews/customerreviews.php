...
<table>
 <tr>
  ...
  <?php
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM reviews R INNER JOIN books B INNER JOIN purchase P INNER JOIN customer C WHERE R.P_ID = P.P_ID AND R.B_ID = B.B_ID AND C.C_ID = R.C_ID";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM reviews R INNER JOIN books B INNER JOIN purchase P INNER JOIN customer C WHERE R.P_ID = P.P_ID AND R.B_ID = B.B_ID AND C.C_ID = R.C_ID LIMIT $offset, $no_of_records_per_page ";
        ...
        echo '</div>';
        echo '<font color="black"><h2 style="text-align:center">Customer Reviews</h2></font>';
...
<?php
 echo "<td class='noborders'><a href='readreview.php?rid=$rid'><p><button class='button'><span>Read</span></button></p></a></td>";
if($row["STATUS"] == 'WAITING FOR APPROVAL'){
 echo "<td class='noborders'><a href='customerreviewsdb.php?rid=$rid'><p><button class='button'><span>Approve</span></button></p></a></td>";
   }
}
?>
</table>
