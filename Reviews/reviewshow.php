...
<table>
  ...
  <?php
        $a = $_SESSION["ID"];
        $sql = "SELECT * FROM reviews R INNER JOIN books B INNER JOIN purchase P WHERE R.P_ID = P.P_ID AND R.B_ID = B.B_ID AND R.C_ID = '$a' AND ( STATUS = 'WAITING FOR APPROVAL' OR STATUS = 'APPROVED' ) ";
        $res_data = mysqli_query($conn,$sql); 
        echo '<font color="black"><h2 style="text-align:center">Your Reviews</h2></font>';
...
echo "<td><a href='http://localhost/buch/Reviews/viewreview.php?id=$id'><p><button class='button'><span>VIEW</span></button></p></a></td>";
echo "<td><a href='http://localhost/buch/Reviews/deletereview.php?id=$id'><p><button class='button'><span>DELETE</span></button></p></a></td>";
