...
<div class="container">
  ...
  <div class="row">
    <div class="column">
     <strong><p class="w3-text-red" style="font-size:20px"><a href="javascript:history.go(-2)"> ⮜⮜ GO TO CART</a></p></strong>
    ...
    <div class="column">
        <?php
             ...
             echo '<p><strong><span style="font-size:20px">Total Items:'.$_SESSION["counter"].'</strong></span></p>';
            $sql3 = "SELECT ID, NAME FROM orders WHERE P_ID = '$pid'";
            $res_data3 = mysqli_query($conn,$sql3);
            $res_data4 = mysqli_query($conn,$sql3);

            echo'<strong> <p style="font-size:20px">Book ID(s):';

            while($row1 = mysqli_fetch_array($res_data3))

              {
                 echo ''.'<strong><span style="font-size:20px">'.$row1["ID"].' , '.'</strong></span>';

               }
               echo ''.'</p></strong>';
               echo '<strong> <p style="font-size:20px">Book Name(s):';

            while($row2 = mysqli_fetch_array($res_data4))
               {
                 echo ''.$row2["NAME"].',';
            ...
            echo '<p><strong><span style="font-size:20px">Total Amount: '.$_SESSION["tp"].'</span></strong></p>';
